...
// Nếu người chơi rời trang (đóng tab / điều hướng sang trang khác) khi ván ĐANG chơi dở và
// chưa settle, tính là thua cược và ghi ngay lập tức — dùng chung cờ payoutSettled với
// settlePayout() ở trên nên mỗi ván tối đa đúng 1 lượt ghi (không bao giờ ghi 2 lần).
...
// Nút "Rời phòng" chỉ gắn sự kiện NẾU tồn tại trong HTML — luôn kiểm tra null trước khi
// addEventListener. Thiếu bước kiểm tra này chính là nguyên nhân gây crash toàn bộ script
// (và làm mất luôn chức năng chọn chế độ) ở các phiên bản trước.
...
const modeCards = document.querySelectorAll('.tl-mode-card');
const betInput = document.getElementById('tl-bet-input');


let selectedMode = null;
let activeBuffPct = 0;
let buffLoadPromise = null; // đảm bảo getActiveBuff() chỉ gọi 1 lần/phiên, các lượt sau dùng lại cache
...
// Tải % buff pet 1 lần duy nhất mỗi phiên (cache lại), tránh đọc Firestore lặp lại mỗi ván.
...
function render() {
  ...
  if (roundBetRow) {
    roundBetRow.style.display = state.over ? 'flex' : 'none';
    if (state.over && roundBetInput) roundBetInput.value = currentBet;
  }
...
startBtn.addEventListener('click', async () => {
  ...
  await loadPetBuff(); // lần đầu sẽ đọc Firestore, các lần sau dùng cache có sẵn
...
window.tlGame = {
  startNewRound() {
    const amt = parseInt(roundBetInput?.value, 10);
    if (!amt || amt < 10) { window.showToast?.('Cược tối thiểu 10 ⭐', 'warn'); return; }
    currentBet = amt;
    currentBuffPct = activeBuffPct; // dùng lại buff đã cache, không đọc Firestore lại
    newGame(currentMode, currentBet, currentBuffPct);
    payoutSettled = false;
    lastTimerTurn = null;
    render();
